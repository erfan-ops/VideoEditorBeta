...
void (ChangePaletteProcessor::* ChangePaletteProcessor::processFunc)() const = nullptr;
void (ChangePaletteProcessor::* ChangePaletteProcessor::processFuncRGBA)() const = nullptr;

cl_kernel ChangePaletteProcessor::m_openclKernel = nullptr;
cl_kernel ChangePaletteProcessor::m_openclKernelRGBA = nullptr;

ChangePaletteProcessor::ChangePaletteProcessor(int size, int nPixels, unsigned char* colorsBGR, int numColors)
...
ChangePaletteProcessor::~ChangePaletteProcessor() {
...
void ChangePaletteProcessor::process() const {
    (this->*processFunc)();
}

void ChangePaletteProcessor::processRGBA() const {
    (this->*processFuncRGBA)();
}

void ChangePaletteProcessor::processCUDA() const {
    cudaMemcpyAsync(d_img, m_img, imgSize, cudaMemcpyHostToDevice, m_cudaStream);

    changePalette_CUDA(gridSize, blockSize, m_cudaStream, d_img, m_nPixels, d_colorsBGR, m_numColors);

    cudaMemcpyAsync(m_img, d_img, imgSize, cudaMemcpyDeviceToHost, m_cudaStream);
    cudaStreamSynchronize(m_cudaStream);
}

void ChangePaletteProcessor::processRGBA_CUDA() const {
    cudaMemcpyAsync(d_img, m_img, imgSize, cudaMemcpyHostToDevice, m_cudaStream);

    changePaletteRGBA_CUDA(gridSize, blockSize, m_cudaStream, d_img, m_nPixels, d_colorsBGR, m_numColors);

    cudaMemcpyAsync(m_img, d_img, imgSize, cudaMemcpyDeviceToHost, m_cudaStream);
    cudaStreamSynchronize(m_cudaStream);
}

void ChangePaletteProcessor::processOpenCL() const {
...
void ChangePaletteProcessor::processRGBA_OpenCL() const {
    // 1. Copy input image to device
    clEnqueueWriteBuffer(globalQueueOpenCL,
        m_imgBuf,
        CL_FALSE,  // Non-blocking write
...
void ChangePaletteProcessor::setImage(const unsigned char* img) {
    memcpy(m_img, img, imgSize);
}

void ChangePaletteProcessor::upload(unsigned char* Dst) {
    memcpy(Dst, m_img, imgSize);
}

void ChangePaletteProcessor::init() {
    if (isCudaAvailable()) {
        ChangePaletteProcessor::processFunc = &ChangePaletteProcessor::processCUDA;
        ChangePaletteProcessor::processFuncRGBA = &ChangePaletteProcessor::processRGBA_CUDA;
    }
    else {
        ChangePaletteProcessor::processFunc = &ChangePaletteProcessor::processOpenCL;
        ChangePaletteProcessor::processFuncRGBA = &ChangePaletteProcessor::processRGBA_OpenCL;

        ChangePaletteProcessor::m_openclKernel = openclUtils::createKernelFromSource(globalContextOpenCL, globalDeviceOpenCL, changePaletteOpenCLKernelSource, "changePalette_kernel");
        ChangePaletteProcessor::m_openclKernelRGBA = openclUtils::createKernelFromSource(globalContextOpenCL, globalDeviceOpenCL, changePaletteRGBAOpenCLKernelSource, "changePaletteRGBA_kernel");
    }
}
