...
class FlatLightProcessor {
public:
    FlatLightProcessor(int size, int nPixels, float lightness);
    ~FlatLightProcessor();

    // Function pointer to process image with either CUDA or OpenCL
    static void (FlatLightProcessor::* processFunc)() const;
    static void (FlatLightProcessor::* processFuncRGBA)() const;
