...
void VFlatLightWorker::process() {
    try {
        ...
        FlatLightProcessor flatLightProcessor(video.getSize(), video.getNumPixels(), m_lightness);
        ...
        while (video.getSuccess()) {
            ...
            flatLightProcessor.setImage(video.getData());
            flatLightProcessor.process();
            flatLightProcessor.upload(frameBuffer.data);
