...
class InverseContrastProcessor {
public:
    InverseContrastProcessor(int size, int nPixels);
    ~InverseContrastProcessor();

    // Function pointer to process image with either CUDA or OpenCL
    static void (InverseContrastProcessor::* processFunc)() const;
    static void (InverseContrastProcessor::* processFuncRGBA)() const;
