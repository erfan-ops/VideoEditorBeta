...
void VChangePaletteWorker::process() {
    try {
        ...
        ChangePaletteProcessor changePaletteProcessor(video.getSize(), video.getNumPixels(), m_colorsBGR, m_numColors);
        ...
        while (video.getSuccess()) {
            ...
            changePaletteProcessor.setImage(video.getData());
            changePaletteProcessor.process();
            changePaletteProcessor.upload(frameBuffer.data);
