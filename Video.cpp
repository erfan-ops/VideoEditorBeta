...
Video::Video(const cv::String& video_file_path, const cv::String& video_output_path, int fourcc) {
    // Open video capture
    video_capture.open(video_file_path);
    if (!video_capture.isOpened()) {
        throw std::runtime_error("Error: Could not open video file: " + video_file_path);
    }

    // Get video properties
    FPS = video_capture.get(cv::CAP_PROP_FPS);
    if (FPS <= 0) {
        throw std::runtime_error("Error: Invalid FPS value in video file: " + video_file_path);
    }
    ...
    if (fourcc != -1) {
        ...
        if (endsWith(video_output_path, ".mp4")) {
            this->fourcc = cv::VideoWriter::fourcc('m', 'p', '4', 'v');
        }
        else if (endsWith(video_output_path, ".avi")) {
            this->fourcc = cv::VideoWriter::fourcc('I', '4', '2', '0');
        }
        else if (endsWith(video_output_path, ".mkv")) {
            this->fourcc = cv::VideoWriter::fourcc('h', 'e', 'v', '1');
        }
        else if (endsWith(video_output_path, ".mov")) {
            this->fourcc = cv::VideoWriter::fourcc('m', 'p', '4', 'v');
        }
        else {
            throw std::invalid_argument("Error: Unsupported video file extension: " + video_output_path);
        }
    }
