...
class LensFilterProcessor {
public:
    LensFilterProcessor(int size, float* passThreshValues, int nThreshValues);
    ~LensFilterProcessor();

    // Function pointer to process image with either CUDA or OpenCL
    static void (LensFilterProcessor ::* processFunc)() const;
    static void (LensFilterProcessor ::* processFuncRGBA)() const;
...
private:
    ...
    float* d_passThreshValues = nullptr;
    ...
    cl_mem m_passThreshValuesBuf = nullptr;
